...
# ── CSS injecté via st.html() qui ne sanitise pas le contenu ─────────────────
# st.markdown() avec unsafe_allow_html peut quand même stripper les <style>.
# st.html() (Streamlit >= 1.32) injecte le HTML brut sans aucun traitement.
_CSS = (
    ...
    "section[data-testid='stSidebar'] .stMultiSelect>div>div{background:rgba(255,255,255,0.12)!important;border:1px solid rgba(255,255,255,0.25)!important;color:#fff!important}"
    "section[data-testid='stSidebar'] label{color:rgba(255,255,255,0.75)!important;font-size:0.78rem!important}"
    "section[data-testid='stSidebar'] hr{border-color:rgba(255,255,255,0.15)!important}"
    ".stat-card{background:#fff;border-left:3px solid #1e4d9b;border-radius:8px;padding:8px 12px 7px;margin-bottom:7px;box-shadow:0 1px 4px rgba(26,39,68,0.07)}"
    ".stat-card h3{margin:0 0 7px 0;font-size:0.7rem;text-transform:uppercase;letter-spacing:1px;color:#1e4d9b;font-weight:700}"
    ".kpi-row{display:flex;gap:6px;flex-wrap:wrap}"
    ".kpi{background:#f4f6fb;border-radius:6px;padding:5px 8px;text-align:center;flex:1;min-width:60px}"
    ".kpi .val{font-size:1.3rem;font-weight:800;color:#1a2744;line-height:1.1}"
    ".kpi .lbl{font-size:0.6rem;color:#6b7a99;text-transform:uppercase;letter-spacing:0.5px;margin-top:1px}"
    ".kpi.green .val{color:#1a7a3c}.kpi.orange .val{color:#d97706}.kpi.red .val{color:#c0272d}.kpi.blue .val{color:#1e4d9b}"
    ".filter-badge{display:inline-block;background:rgba(45,101,196,0.1);color:#1e4d9b;border:1px solid rgba(45,101,196,0.3);border-radius:5px;padding:2px 8px;font-size:0.7rem;font-weight:600;margin-bottom:6px}"
    ".player-header{background:linear-gradient(120deg,#1a2744 0%,#1e4d9b 100%);border-radius:10px;padding:10px 14px;display:flex;align-items:center;gap:14px;margin-bottom:10px}"
    ".player-header img{border-radius:8px;border:2px solid rgba(255,255,255,0.4);object-fit:cover;width:90px;height:108px;flex-shrink:0}"
    ".player-no-photo{width:90px;height:108px;border-radius:8px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center;font-size:2.5rem;border:2px solid rgba(255,255,255,0.3);flex-shrink:0}"
    ...
    ".player-info .poste{display:inline-block;margin-top:4px;background:#c0272d;color:#fff;font-size:0.65rem;font-weight:700;letter-spacing:1px;text-transform:uppercase;border-radius:4px;padding:2px 7px}"
    ".js-plotly-plot .plotly{touch-action:pan-y!important}.plotly-graph-div{touch-action:pan-y!important}"
    # Bouton flottant pour rouvrir la sidebar sur mobile (visible uniquement quand sidebar fermée)
    ".sidebar-toggle-btn{"
    "display:none;position:fixed;top:12px;left:12px;z-index:999999;"
    "background:#1a2744;color:#fff;border:none;border-radius:8px;"
    "width:42px;height:42px;font-size:1.3rem;cursor:pointer;"
    "box-shadow:0 2px 8px rgba(0,0,0,0.35);align-items:center;justify-content:center;}"
    "@media(max-width:768px){"
    ".sidebar-toggle-btn{display:flex!important}}"
    "@media(max-width:768px){"
    ".block-container{padding:3.5rem 0.5rem 0.4rem!important}"
    ...
    "</style>"
    # Bouton HTML + JS pour cibler le bouton natif Streamlit de la sidebar
    "<button class='sidebar-toggle-btn' onclick=\""
    "(document.querySelector('[data-testid=\\'stSidebarCollapsedControl\\'] button')"
    "||document.querySelector('[data-testid=\\'stSidebarContent\\'] ~ * button')"
