...
# ── CSS global (string ordinaire, PAS f-string, pour éviter tout conflit
#    entre les accolades CSS et l'interpolation Python) ─────────────────────
_CSS = """
...
<style>
  ...
  .stat-card h3 {
    ...
    letter-spacing: 1px; color: #1e4d9b; font-weight: 700;
  }
  .kpi-row { display: flex; gap: 6px; flex-wrap: wrap; }
  .kpi {
    background: #f4f6fb; border-radius: 6px; padding: 5px 8px;
  ...
  .kpi.red .val    { color: #c0272d; }
  .kpi.blue .val   { color: #1e4d9b; }

  /* Badge filtre */
  .filter-badge {
    display: inline-block; background: #2d65c422; color: #1e4d9b;
    border: 1px solid #2d65c455; border-radius: 5px; padding: 2px 8px;
    font-size: 0.7rem; font-weight: 600; margin-bottom: 6px;
  }

  /* Header joueur */
  .player-header {
    background: linear-gradient(120deg, #1a2744 0%, #1e4d9b 100%);
